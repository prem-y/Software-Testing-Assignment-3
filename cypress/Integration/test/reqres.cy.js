describe("ReqResAPI", () => {
  ...
  it("POST - Create User", () => {
    cy.request({
      ...
      expect(res).to.have.property("name", "Prem");
      expect(res).to.have.property("job", "Engineer");
  ...
  it("PUT - Update User", () => {
    cy.request({
      ...
      const res = response.body;
      expect(response.status).to.eql(200);
      expect(res).to.have.property("name", "Prem Updated");
      expect(res).to.have.property("job", "Senior Engineer");
      cy.log(res);
    });
  });
  ...
  it("PATCH - Update User", () => {
    cy.request({
      ...
      const res = response.body;
      expect(response.status).to.eql(200);
      expect(res).to.have.property("job", "Lead Engineer");
      cy.log(res);
    });
  });
